...
//Увеличение масштаба по сторонам от val(maxZoom) на range пикселей с плавным заполнением остального
void zoomRegion(    
    CRGB* leds,     //массив с лентой
    CRGBPalette16 targetPalette,        //палитра по которой заливают
    uint8_t val,       //позиция ценртального пикселя, с максимальным масштабом
    uint8_t range,      //расстояние от точки с максимальным значением, до точки, где эначение равно нулю
    uint8_t maxZoom,        //(0-255) максимальный зум * 1/16 ( 8=2:1 ; 16 = 1:1 ; 32 = 2:1 ; 255 = (15+15/16):1 )
    uint8_t num_leds,     //количество диодов в массиве
    uint8_t startPixel) {     //номер диода на ленте, который будет началом   

    uint16_t centerPixel = val * num_leds / 255;
    uint16_t startIndex = (centerPixel + num_leds - range) % num_leds;
    uint16_t endIndex = (centerPixel + range) % num_leds;
    ...
    for (uint8_t i = 0; i <= range; i++){
        ...
        uint8_t LedDist = 16*16/scale;

        uint16_t pixelIndex = (centerPixel + i) % num_leds;
        mapLedInPallette[pixelIndex] = LedDist;
...
/*         Serial.print("mapLedInPallette[");
    ...
    foolLedDist += val * mapSum * 2 / 255;
    for (uint16_t i = 0 ; i < num_leds ; i++){
        uint16_t pixIndex = ( i + centerPixel ) % num_leds;
        uint16_t nextIndex = ( i + 1 + centerPixel ) % num_leds;
        uint8_t mapLedDist = foolLedDist * 255 / (2 * mapSum) ;
/*         Serial.print("pixIndex = ");
        Serial.print(pixIndex);
        Serial.print("  >>  foolLedDist = ");
        Serial.print(foolLedDist); */

        foolLedDist = (foolLedDist + mapLedInPallette[pixIndex] + mapLedInPallette[nextIndex] ) % (2 * mapSum) ;
...
void choseFromPal(    
    ...
    uint16_t startPixel){      //номер диода на ленте, который будет началом       

    uint16_t tailLenght = (val * num_leds / 255); 
    ...
    case RIGHT:
        lightDirection = 1;
        tailDirection = 0;
        headeIndex = (startPixel + tailLenght * lightDirection + num_leds) % num_leds;
        break;
    case LEFT:
        lightDirection = -1;
        tailDirection = 0;
    ...
    for(uint8_t i = 0; i < tailLenght ; i++ ){
        uint8_t iIndex = (lightDirection * i  + startPixel + num_leds + tailDirection * tailLenght) % num_leds;
